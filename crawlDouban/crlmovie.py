...
class crawlMovies():
	...
	def crawler(self,seed_url,link_regex,max_threads=10):
		...
		while threads or crawl_queue:#如果线程池不为空或者队列不为空(说明还有任务没有执行完)
			...
			if len(threads) < max_threads and crawl_queue:#任务列表不为空且线程数小于最大线程数，新建线程
				thread = threading.Thread(target=process_url,args=(crawl_queue,seen))
	...
	def process_url(self,crawl_queue,seen):
		url = crawl_queue.pop()
...
if __name__ == '__main__':
	...
	# cm = crawlMovies()
	# html = cm.crawler(seed_url,pattern)
